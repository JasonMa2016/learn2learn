...
def fast_adapt(batch, learner, loss, adaptation_steps, shots, ways, device):
    ...
    for step in range(adaptation_steps):
        ...
        if step == 0:
            local_loss = train_error.clone().detach().item()
    ...
    return valid_error, valid_accuracy, local_loss
...
def main(args):
    ...
    for iteration in range(num_iterations):
        ...
        for task in range(meta_batch_size):
            ...
            evaluation_error, evaluation_accuracy, F_k = fast_adapt(batch,
                                                               learner,
                                                               loss,
                                                               adaptation_steps,
                                                               shots,
            ...
            for p1, p2 in zip(learner.parameters(), maml.parameters()):
                ...
                current = lipschitz * (p1_copy-p2_copy)
                delta_w.append(current)
                delta.append(temp * q * current)

            delta_w_norm = 0
            for params in delta_w:
                delta_w_norm += params.data.norm() ** 2
            h_k = q * temp * delta_w_norm + lipschitz * temp * q
            h_total += h_k
            ...
            evaluation_error, evaluation_accuracy, F_k_test = fast_adapt(batch,
                                                               learner,
                                                               loss,
                                                               adaptation_steps,
                                                               shots,
